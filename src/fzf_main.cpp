...
PYBIND11_MODULE(fuzzyfind_functions, m) {
    ...
    m.def("fuzzy_search", [](string query, string root_path, size_t max_buffer_size=50*KB, size_t score_threshold=0, bool no_retry=false)
            {
            if (score_threshold == 0) {
                score_threshold = _calculate_score_threshold(query);
            }
            vector<fs::path> path_list = fuzzy_search(query, root_path, max_buffer_size, score_threshold, false);
